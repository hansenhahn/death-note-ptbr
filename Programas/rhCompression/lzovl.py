...
def uncompress(infile):
    ...
    header3, header2, header1 = struct.unpack('<HHL', infile.read(8))
    
    size = header1 + header3
    start = eof - ( header2 >> 8 )
    infile.seek( 0, 0 )
    buffer = infile.read(start)
    fd = mmap.mmap(-1, len(buffer))
    fd.write( buffer[::-1] )
    fd.seek(0,0)
    ...
    while True:
        ...
        for x in range(8):
            if flag & 0x80:
                lz_par = struct.unpack('>H',fd.read(2))[0]
                lz_size = ((lz_par >> 12) & 0xF) + 2 + 1  # pq é maior ou igual a 0
                lz_distance = (lz_par & 0xFFF) + 2 + 1
                for x in range(lz_size):
                    decoded_buffer.append(decoded_buffer[len(decoded_buffer) - lz_distance])
            else:
                d = fd.read(1)
                decoded_buffer.append(d)
            if len(decoded_buffer) >= size:
                while len(decoded_buffer) > size:
                    decoded_buffer.pop()
                return decoded_buffer[::-1]
            flag <<= 1
